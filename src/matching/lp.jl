...
function maximum_weight_maximal_matching{T<:Real}(g::Graph, w::Dict{Edge,T}, cutoff; solver = GLPKSolverMIP)
    wnew = Dict{Edge,T}()
    for (e,x) in w
        if x >= cutoff
            wnew[e] = x
...
function maximum_weight_maximal_matching{T<:Real}(g::Graph, w::Dict{Edge,T}; solver = GLPKSolverMIP)
